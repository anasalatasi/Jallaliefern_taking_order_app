...
import 'package:jallaliefern_taking_orders_app/widgets/table_reservation_tile.dart';
...
class _TableReservationState extends State<TableReservationPage>
    with WidgetsBindingObserver {
  RefreshController _refreshController =
      RefreshController(initialRefresh: true);
  List<TableReservation> tableReservations = List<TableReservation>.empty();
  @override
  void initState() {
    super.initState();
  ...
  Future<void> _onRefresh() async {
    ...
    try {
      final tmp = await locator<ApiService>().getTableReservations();
      setState(() {
        tableReservations = tmp;
      });
  ...
  Widget build(BuildContext context) => SmartRefresher(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          return TableReservationTile(
              tableReservation: tableReservations[index],
              parentRefresh: _onRefresh);
        },
      ));
}
